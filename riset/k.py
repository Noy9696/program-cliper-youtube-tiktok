...
vf = (
    ...
    # === VIDEO UTAMA (CROP TENGAH -s> SCALE + BORDER HITAM) ===
    ...
    # === CROP GAMER (GESER KE KANAN DIKIT + BORDER HITAM) ===
    "[0:v]crop=iw*0.25:ih*0.25:iw*0.73:ih*0.70,"
    "scale=450:450,"
    "drawbox=x=0:y=0:w=iw:h=ih:color=black:t=6[gamer];"

    # === COMPOSE ===
    # 1. Overlay main video ke background blur (tengah horizontal, agak ke atas)
    "[bg][main]overlay=(W-w)/2:150[tmp];"
    
    # 2. Overlay gamer ke pojok kanan bawah (di area blur)
    "[tmp][gamer]overlay=W-w-40:H-h-40[v]"
)
